...
const questions = [
  ...
  },
  {
    ...
    language: "C++",
    title: "Main Function",
    description: "Complete the main function return type.",
  ...
  },
  {
    ...
    language: "Java",
    title: "Main Method",
    description: "Fill in the correct return type for the Java main method.",
...
const GamePage = () => {
  ...
  // Timer: initialised on first game entry, not on landing page
  useEffect(() => {
  ...
  // Click-to-fill fallback for mobile / accessibility
  const handleOptionClick = (option) => {
    const firstEmpty = filledSlots.findIndex((s) => s === "");
    if (firstEmpty === -1) return;
    setFilledSlots((prev) => {
      const updated = [...prev];
      updated[firstEmpty] = option;
      return updated;
    });
  };
  ...
  const handleNext = () => {
    // 1 point per correct answer slot
    const correct = filledSlots.every(
      (val, i) => val === currentQuestion.correctAnswers[i]
    );
    const points = correct ? 1 : 0;
    const updatedScore = score + points;
    setScore(updatedScore);
    setAnsweredCorrect((prev) => [...prev, correct]);
  ...
  const timerClass =
    timeLeft < 60000 ? "round-one-timer timer-danger" : "round-one-timer";
      ...
      <div className="round-one-game">
        ...
        <div className="round-one-progress">
          <div
            ...
            style={{
              width: `${((currentIndex + (isComplete ? 1 : 0)) / questions.length) * 100}%`,
            }}
          />
        </div>
        ...
        {isComplete ? (
          <div className="round-one-complete">
            ...
            {user && <p className="complete-name">{user.name}</p>}
            <p>
              {hasTimeExpired
                ? "Time expired. Here's your final score:"
                : "Great work! Here's your final score:"}
            </p>
            <strong className="complete-score">
              {score} / {questions.length}
            </strong>
            <p className="complete-sub">
              {answeredCorrect.filter(Boolean).length} correct out of{" "}
              {answeredCorrect.length} answered
            </p>
            <button
              type="button"
          ...
          <>
            <div className="round-one-question-meta">
              <div className="round-one-question-top">
                <p>
                  Question {currentIndex + 1} / {questions.length}
                </p>
                <span className="round-one-lang-badge">
                  ...
                  {codeSegments.map((segment, index) => (
                    <span key={`segment-${index}`}>
                      ...
                      {index < currentQuestion.correctAnswers.length && (
                        <span
                          className={`round-one-slot${filledSlots[index] ? " filled" : ""}`}
                          onDrop={(event) => handleDrop(index, event)}
                          onDragOver={handleDragOver}
                          onClick={() =>
                            filledSlots[index] && handleSlotClick(index)
                          }
                          title={
                            filledSlots[index]
                              ? "Click to clear"
